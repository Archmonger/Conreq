...
DEV_PACKAGES_DIR = getattr(settings, "DEV_PACKAGES_DIR")
...
class Command(TemplateCommand):
    ...
    def handle(self, app_name: str, **options):  # type: ignore
        ...
        name = app_name
        app_or_project = "app"
        target = str(DEV_PACKAGES_DIR / package_name / "apps" / "")
        ...
        print(
            f"Successfully created new app {app_name} in '{Path(target) / app_name}'."
        )
